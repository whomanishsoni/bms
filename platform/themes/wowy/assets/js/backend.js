(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).ready(function () {
            ...
            document.addEventListener('ecommerce.quick-shop.completed', (event) => {
                const { modal } = event.detail;

                // Override attribute selection in modal to prevent URL changes
                modal.find('.product-attributes .product-attribute-item').off('click').on('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();

                    const $this = $(this);
                    const $parent = $this.closest('.attribute-swatches-wrapper');

                    // Toggle selection
                    $parent.find('.product-attribute-item').removeClass('active');
                    $this.addClass('active');

                    // Trigger swatch change event without URL update
                ...
                // Override filter item links in modal
                modal.find('.list-filter a').off('click').on('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                ...
                // Fix quantity buttons to prevent navigation
                modal.find('.detail-qty .qty-up, .detail-qty .qty-down').each(function() {
                    const $this = $(this);
                    if ($this.attr('href') === '#') {
